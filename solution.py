...
def checkerboard(size):
    ...
    for i in range(0, 8):
        for j in range(0, 8):
            if ((i+j) % 2 == 1):
                c = c + 1
                image[i*side:i*side + side, j*side:j*side + side] = 255
    return image
"""
...
def nn_interpolation(source, new_size):
    ...
    height_source, width_source = source.shape
    x = width_source / width
    y = height_source / height
    image = np.zeros(new_size).astype(np.uint8)
    for (i, j) in np.ndindex(image.shape):
        image[i, j] = source[int(i*y), int(j*x)]
    return image
